...
export class TokenDetailsHandler {
  ...
  async showTokenDetails(chatId, userInfo, tokenData) {
    const loadingMsg = await this.bot.telegram.sendMessage(chatId, '🪙 Loading token details...');
  ...
  async executeApproval(chatId, userInfo, tokenAddress, network) {
    const loadingMsg = await this.bot.telegram.sendMessage(chatId, '🔄 Processing approval...');
