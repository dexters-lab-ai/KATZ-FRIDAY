...
export class NotificationsCommand extends Command {
  ...
  async showPriceAlerts(chatId) {
    try {
      ...
      await this.bot.telegram.sendPhoto(chatId, { source: canvasBuffer }, { caption: "*Your Price Alerts* 💰", parse_mode: "Markdown" });
    } catch (error) {
      await ErrorHandler.handle(error, this.bot, chatId);
    }
  }
  ...
  async showReminders(chatId) {
    try {
      ...
      await this.bot.telegram.sendPhoto(chatId, { source: canvasBuffer }, { caption: "*Your Reminders* ⏰", parse_mode: "Markdown" });
    } catch (error) {
      await ErrorHandler.handle(error, this.bot, chatId);
    }
  }
  ...
  async showTimedOrders(chatId) {
    try {
      ...
      await this.bot.telegram.sendPhoto(chatId, { source: canvasBuffer }, { caption: "*Your Timed Orders* ⚡", parse_mode: "Markdown" });
    } catch (error) {
      await ErrorHandler.handle(error, this.bot, chatId);
    }
  }
  ...
  async processShareNotification(chatId, input) {
    try {
      ...
      await this.bot.telegram.sendMessage(chatId, `✅ ${type} successfully shared with ${username} for ${expiration} hours!`);
    } catch (error) {
      await ErrorHandler.handle(error, this.bot, chatId);
    } finally {
  ...
  async showNotificationSettings(chatId) {
    try {
      ...
      await this.bot.telegram.sendMessage(chatId, `*Notification Settings* ⚙️\n\nCustomize your notification preferences:`, {
        parse_mode: "Markdown",
        reply_markup: keyboard,
      });
    } catch (error) {
